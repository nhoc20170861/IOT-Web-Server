...
async function createWorkerRobots() {
    for (const key in queueRobots) {
        ...
        myWorkerRobots[key].on('completed', (job, value) => {
            ...
            Logging.info(`robot1: Completed job with jobId ${job.data.taskId}, ${value}`);
        });
        // Cấu hình retry cho job thất bại
        myWorkerRobots[key].on('failed', (job, err) => {
            Logging.warning(`robot1: Job ${job.taskId} failed with error: ${err.message} and retry with ${job.attemptsMade}`);
        });
...
createWorkerRobots();
// Object.keys(queueRobots).forEach(async (key) => {
//     await queueRobots[key].resume();
//     myWorkerRobots[key] = new Worker(
//         key,
//         async (job) => {
