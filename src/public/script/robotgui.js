...
const handleWheelEvent = (event) => {
    ...
    if (event.wheelDeltaY) {
        current_scale = event.wheelDeltaY > 0 ? prev_scale * 1.05 : prev_scale / 1.05;
        prev_scale = current_scale;
    }

    viewer.scaleToDimensions(scaleDim.width / current_scale, scaleDim.height / current_scale);
    viewer.shift((scaleDim.x + shift.xSum) / current_scale, (scaleDim.y + shift.ySum) / current_scale);
};
// Connecting to ROS
// -----------------
...
ros.on('connection', async function () {
    ...
    await init();
    document.getElementById('map_ros').addEventListener('wheel', handleWheelEvent);
});
...
ros.connect('ws://192.168.1.180:9090');
...
async function init() {
    ...
    gridClient.on('change', function () {
        ...
        scaleDim.height = (gridClient.currentGrid.width / window.innerWidth) * window.innerHeight;
        scaleDim.x = gridClient.currentGrid.pose.position.x;
        scaleDim.y = (scaleDim.height / gridClient.currentGrid.width) * gridClient.currentGrid.pose.position.y;
    ...
    let Mir = {
        model: '',
        pathTopic: '/move_base_node/SBPLLatticePlanner/plan',
        name: '/',
        fillColor: createjs.Graphics.getRGB(255, 128, 0.66)
    };

    Mir['model'] = new ROS2D.Robot({
        strokeColor: createjs.Graphics.getRGB(255, 0, 0),
        fillColor: Mir.fillColor,
        width: 0.3,
        height: 0.25
    });
    Mir['initPoseListener'] = new ROSLIB.Topic({
        ros: ros,
        name: '/initialpose',
        messageType: 'geometry_msgs/PoseWithCovarianceStamped',
        throttle_rate: 100
    ...
    Mir['poseTopic'] = new ROSLIB.Topic({
        ros: ros,
        name: '/amcl_pose',
        messageType: 'geometry_msgs/PoseWithCovarianceStamped'
    });
    ...
    // ===== create path view =========
    Mir['pathView'] = new ROS2D.PathShape({
        strokeSize: 0.04,
        strokeColor: '#' + Math.floor(Math.random() * 16777215).toString(16)
    });
    Mir['pathGlobalTopic'] = new ROSLIB.Topic({
        ros: ros,
        name: Mir.pathTopic,
        messageType: 'nav_msgs/Path'
    });
    gridClient.rootObject.addChild(Mir.model);
    initSubscribePose(Mir.poseTopic, Mir.model);
    initSubscribePose(Mir.initPoseListener, Mir.model);

    gridClient.rootObject.addChild(Mir.pathView);
    initSubscribePathPlan(Mir.pathGlobalTopic, Mir.pathView);
}
