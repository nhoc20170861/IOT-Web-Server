var ROS2D = (function (exports, createjs, ROSLIB) {
    ...
    class Robot  extends createjs__namespace.Shape{
        constructor(options){
            options = options || {};
            let width = options.width || 0.25;
            let height = options.height || 0.2;
            let strokeSize = options.strokeSize || 0.02;
            let strokeColor = options.strokeColor || createjs__namespace.Graphics.getRGB(255, 0, 0);
            let fillColor = options.fillColor || createjs__namespace.Graphics.getRGB(255, 128, 0.66);
            // let pulse = options.pulse;
          
            // draw the arrow
            let graphics = new createjs__namespace.Graphics();
            // line width
            graphics.setStrokeStyle(strokeSize);
            graphics.moveTo(0, 0);
            graphics.beginStroke(strokeColor);
            graphics.beginFill(fillColor);
            graphics.lineTo(0, height / 2.0);
            graphics.lineTo(width / 2.0, height / 2.0);
            graphics.lineTo(width / 2.0, -height / 2.0);
            graphics.lineTo(-width / 2.0, -height / 2.0);
            graphics.lineTo(-width / 2.0, height / 2.0);
            graphics.lineTo(0, height / 2.0);
          
            graphics.moveTo(width / 2.0, -0 / 2.0);
            graphics.lineTo(0, -0 / 2.0);
    ...
    class NavigationArrow extends createjs__namespace.Shape {
      ...
      constructor(options) {
        ...
        var strokeSize = options.strokeSize || 0.02;
        var strokeColor = options.strokeColor || createjs__namespace.Graphics.getRGB(0, 0, 0);
        var fillColor = options.fillColor || createjs__namespace.Graphics.getRGB(255, 0, 0);
        var pulse = options.pulse;
    ...
    class NavGoal {
      ...
      constructor(options) {
        ...
        this.container = new createjs__namespace.Container();
        this.rootObject.addChild(this.container);
  
        // marker for goal orientation
        this.goalOrientationMarker = new ArrowShape({
          size: 30,
          strokeSize: 1,
          fillColor: createjs__namespace.Graphics.getRGB(0, 255, 0, 0.66),
          pulse: false,
        });
